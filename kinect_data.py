...
import numpy as np
import cv2
import Camera_Calibration_Infrared
import Camera_Calibration_Color


stream = "undistored_Depth"
# raw_Depth, raw_IR, raw_Color, undistored_Color, undistored_Depth, undistored_IR, Color_Ali
...
def color_alignment(kinect_color, mtx, dist, newcameramtx):
    ...
    while True:
        if kinect_color.has_new_color_frame():
            ...
            for i in range(20):
                full_color_frame = cv2.line(full_color_frame, (int(width / 2) + X * i, 0), (int(width / 2) + X * i, height), (100, 100, 100))
                full_color_frame = cv2.line(full_color_frame, (int(width / 2) - X * i, 0), (int(width / 2) - X * i, height), (100, 100, 100))
            for i in range(20):
                full_color_frame = cv2.line(full_color_frame, (0, int(height / 2) + Y * i), (width, int(height / 2) + Y * i), (100, 100, 100))
                full_color_frame = cv2.line(full_color_frame, (0, int(height / 2) - Y * i), (width, int(height / 2) - Y * i), (100, 100, 100))
